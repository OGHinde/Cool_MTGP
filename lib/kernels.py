...
The code in this file uses parts of the sklearn kernels library as a starting point.
...
class KernelOperator(Kernel):
    ...
    def setMask(self, mask):
        self.k1.setMask(mask)
        self.k2.setMask(mask)
...
class ConstantKernel(StationaryKernelMixin, Kernel):
    ...
    """
    def __init__(self, constant_value=1.0, constant_value_bounds=(1e-5, 1e5), mask = None):
        self.constant_value = constant_value
        self.constant_value_bounds = constant_value_bounds
        self.mask =mask # It is only defined, but not used for this kernel
...
class WhiteKernel(StationaryKernelMixin, Kernel):
    ...
    """
    def __init__(self, noise_level=1.0, noise_level_bounds=(1e-5, 1e5), mask = None):
...
class RBF(StationaryKernelMixin, NormalizedKernelMixin, Kernel):
    ...
    mask : boolean array  (n_features)
            It indicates the features to be used by this kernel. For example, if there are 
            four features, the mask [True,True,False,False] would force to use only the first
            two features.
    ...
    """
    def __init__(self, length_scale=1.0, length_scale_bounds=(1e-5, 1e5),mask=None):
        self.mask = mask
...
class DotProduct(Kernel):
    ...
    def __init__(self, sigma_0=1.0, sigma_0_bounds=(1e-5, 1e5),mask = None):
    ...
    def __call__(self, X, Y=None, eval_gradient=False):
        ...
        if self.mask is not None:
            try:
                ...
                raise ValueError(
                    "Kernel mask does not match X dimensions.")
        if Y is None:
