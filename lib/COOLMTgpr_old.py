...
from scipy.linalg import cholesky, cho_solve, solve_triangular, pinv
from sklearn.base import clone
from sklearn.utils.validation import check_X_y

from lib.kernels import DotProduct, WhiteKernel, RBF, ConstantKernel
from lib.gpr import GaussianProcessRegressor
...
class MultitaskGP:
    ...
    def __init__(self, kernel=None, kernel_noise = None, alpha=1e-10,
    ...
    def fit (self, X, y):
        ...
        for t in range(self.__T):
            ...
            if t>0:
                ...
                kernel_t = deepcopy(self.kernelX_*kernelSignal + self.kernelNoise_)
        ...
        if self.__optimizer is not None:
            ...
            print('Parameter values have been learnt: %s' %np.exp(optima[np.argmin(lml_values)][0]))
            print(np.min(lml_values))         
        ...
        self.get_MT_Matrix()
        print('The prior of W and noise matrix have been generated')

        return self

    
    # def __MT_Matrix_1(self):
    ...
    #         if tt==0:
    #             # First element is the noise and prior of gp 0
    #             SigmaTT[tt,tt] = NoiseValues[tt] 
    #             PriorW[tt,tt] = AValues[tt] 
                        
    #         else:       
    #             # Compute the value of wyt
    #             wyt = gp.alpha_ @ gp.X_train_[:,self.__D:]

    #             # Compute noise elements
    #             sigma_col = wyt.dot(SigmaTT[:tt,:tt])
    #             SigmaTT[tt,:tt] = sigma_col 
    #             SigmaTT[:tt,tt] = sigma_col
    #             SigmaTT[tt,tt] = NoiseValues[tt] + wyt.dot(SigmaTT[:tt,:tt]).dot(wyt)
                
    #             # Compute prior elements
    #             Prior_col = wyt.dot(PriorW[:tt,:tt])
    #             PriorW[tt,:tt] = Prior_col 
    #             PriorW[:tt,tt] = Prior_col
    #             PriorW[tt,tt] = AValues[tt]  + wyt.dot(PriorW[:tt,:tt]).dot(wyt)
    ...
    def __MT_Matrix(self, K_tr):
        ...
        for tt, gp in enumerate(self.__listGPs):
            ...
            Mat_alpha_prior[tt,:] = np.sqrt(AValues[tt]) * gp.alpha_ /np.sqrt(wwT)
        
        B =  np.linalg.pinv(np.eye(T)-Mat_wyt) 
        #Mat_W = B @  Mat_wxt
    ...
    #     if data['kernel']=='lineal':
    #     K_tr = np.dot(X_tr,X_tr.T)
    #     K_tr_test = np.dot(X_tr,X_test.T) 
    #     K_test = np.dot(X_test,X_test.T) 

    #     #C=COpt
    #     #Sigma = SigmaOpt
    #     C_K_tr = np.kron(C, K_tr)
    #     C_K_tr_test = np.kron(C, K_tr_test)
    #     C_K_test = np.kron(C, K_test)
    #     Sigma_I = np.kron(Sigma, np.eye(N))
    ...
    #     mpred = C_K_tr.T @  Inverse  @ y_tr.T.ravel()
    #     #y_var = C_K_test - np.einsum("ij,ij->i", np.dot(C_K_tr_test.T, Inverse), C_K_tr_test.T)
    #     y_var = C_K_tr - C_K_tr.T @  Inverse  @ C_K_tr

    #     MSE_GP =np.mean((y_tr-mpred.reshape((T,N)).T)**2, axis=0)
    #     print('MSE train kronsum:')
    #     print(MSE_GP)

    #     # Compute mean and std of the MT-GP
    #     mpred = C_K_tr_test.T @  Inverse  @ y_tr.T.ravel()
    #     #y_var = C_K_test - np.einsum("ij,ij->i", np.dot(C_K_tr_test.T, Inverse), C_K_tr_test.T)
    #     y_var = C_K_test - C_K_tr_test.T @  Inverse  @ C_K_tr_test

    #     MSE_GP_test =np.mean((y_test-mpred.reshape((T,N_test)).T)**2, axis=0)

    #     print('MSE test kronsum:')
    #     print(MSE_GP_test)


    #     KronSum_train = MSE_GP
    #     KronSum_test = MSE_GP_test
    ...
    #     mpred = np.zeros((Ntest,T))
    #     covpred = np.zeros((T, Ntest, Ntest))
    #     Mat_wyt = np.zeros((T,T))

    #     for tt, gp in enumerate(self.__listGPs):
            
    #         Z = np.hstack((X, mpred[:,:tt]))
    #         mpred[:,tt], covpred[tt,:,:] = gp.predict(Z, return_cov=True)
    #         Mat_wyt[tt,:tt] = gp.alpha_ @ gp.X_train_[:,self.__D:]
       

    #     # Compute covariance between tasks. TODO efficient!!!!!
    #     allcovpred = np.zeros((Ntest*T,Ntest*T))
    #     for t in range(T):
    #         allcovpred[Ntest*t:Ntest*(t+1),Ntest*t:Ntest*(t+1)] = covpred[tt,:,:]
    #         for tt in range(t-1):
    #             for ii in range(t-1):
    #                 allcovpred[Ntest*t:Ntest*(t+1),Ntest*tt:Ntest*(tt+1)] +=  Mat_wyt[t,ii] * allcovpred[Ntest*ii:Ntest*(ii+1),Ntest*tt:Ntest*(tt+1)]
    #         allcovpred[:Ntest*t,Ntest*t:Ntest*(t+1)] = allcovpred[Ntest*t:Ntest*(t+1),:Ntest*t].T


    #     return mpred, allcovpred, covpred

    def predict(self, X, conf_intervals=False):
        ...
        Inverse = np.linalg.pinv(C_K_tr + Sigma_I + np.eye(Sigma_I.shape[0])*1e-10)
        ...
        if conf_intervals:
            ...
            for t in range(T):
                ...
                Inverse = pinv(C_K_tr + Sigma_I + np.eye(Sigma_I.shape[0])*1e-15)
    ...
    # def predict_bad(self, X):
        
    #     N = self.__X.shape[0]
    #     Ntest = X.shape[0]
    #     T = self.__y.shape[1]
    #     # To compute the intratask covariance (over the training data)
    #     Cov_Y = np.zeros((N*T,N*T))
    #     # To compute the cross covariance training data and predicted ouput
    #     Cov_Yf = np.zeros((N*T,Ntest*T))
    #     # We complete it with the noise elements
    #     noise = self.JointDistributionY_Cov()
    #     for tt in range(T):
    #         values = np.repeat(noise.diagonal(tt), N) 
    #         index_i = np.arange(0,T*N-tt*N)
    #         index_j = np.arange(tt*N,T*N)
            
    #         Cov_Y[index_i, index_j] =values
    #         Cov_Y[index_j, index_i] =values
        

    #     gp = self.__listGPs[0]
    #     A = gp.kernel_(gp.X_train_)
    #     A[np.diag_indices_from(A)] += gp.alpha

    #     K_test= gp.kernel_(gp.X_train_, X) 
    #     #for tt, gp in enumerate(self.__listGPs):
    #     for tt in range(T):
            
    #         Cov_Yf[tt*N:(tt+1)*N, tt*Ntest:(tt+1)*Ntest] = K_test      
            
    #         #L_inv = solve_triangular(gp.L_.T, np.eye(gp.L_.shape[0]))
    #         #A_1 = L_inv.dot(L_inv.T)
    #         #A0_1 = np.linalg.inv(A0)
    #         Cov_Y[tt*N:(tt+1)*N, tt*N:(tt+1)*N] += A

    #     Cov_Y_1 = np.linalg.inv(Cov_Y)
    #     mpred = Cov_Yf.T @ Cov_Y_1 @ self.__y.T.ravel()

    #     y_var = np.tile(gp.kernel_.diag(X),(1,T))
        
    #     K_test_T = np.tile(K_test,(T,1))

    #     y_var -= np.diag(Cov_Yf.T@ Cov_Y_1@ Cov_Yf)
    ...
    #     #Vpred = Cov_Yf.T @ np.linalg.inv(Cov_Y) @ self.__y
    #     return mpred.reshape((T,Ntest)).T, np.sqrt(y_var).reshape((T,Ntest)).T
