...
class MultitaskGP:
    ...
    def setThetas(self, theta_opt):
        ...
        self.kernelX_ = deepcopy(self.__listGPs[0].kernel_)
    ...
    def predict(self, X, mode='efficient', conf_intervals=False):
        ...
        if mode=='efficient':
            ...
            mpred = (K_tr_test.T @(u_K@vect_y_tr_hat.reshape((T,N)).T@u_C2.T@(u_sigma*np.divide(1,np.sqrt(s_sigma))).T)@PriorW).T.ravel()
